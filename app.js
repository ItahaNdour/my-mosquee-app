...
function applyMosqueLockFromQuery() {
  const mid = getQueryParam('m');
  if (!mid) return false;

  const mosques = loadMosques();
  const exists = mosques.some((x) => x.id === mid);
  if (!exists) return false;

  setCurrentMosque(mid);
...
/* Donations storage (objectif MENSUEL) */
...
function renderDonationAdmin() {
  ...
  const left = Math.max(0, goal - month);
...
function renderDonationPublic() {
  const m = getCurrentMosque();
  const goal = getGoal(m);
  const month = monthSum(m);

  const goalEl = el('don-public-goal');
  const monthEl = el('don-public-month');
  const pctEl = el('don-public-pct');
  const barEl = el('don-public-bar');

  if (goalEl) goalEl.textContent = goal.toLocaleString('fr-FR');
  if (monthEl) monthEl.textContent = month.toLocaleString('fr-FR');
...
function countPendingDonations() {
  const m = getCurrentMosque();
  return loadList(m).filter((x) => x.status === 'pending').length;
}

function updateAdminBadge() {
  const b = el('admin-badge');
  if (!b) return;
  const n = countPendingDonations();
  if (n > 0) {
...
function setup() {
  ...
  const locked = applyMosqueLockFromQuery();
